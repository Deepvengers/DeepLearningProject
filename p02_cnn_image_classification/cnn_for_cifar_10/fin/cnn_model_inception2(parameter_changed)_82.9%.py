...
class Model:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            with tf.name_scope('conv_layer1') as scope:
                ...
                self.L1 = tf.nn.max_pool(value=self.L1_sub, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                                         padding='SAME')  # 26*26 -> 13*13
                # print(self.L1.shape)
            ...
            with tf.name_scope('conv_layer3') as scope:
                self.W3_sub = tf.get_variable(name='W3_sub', shape=[3, 3, 160, 160], dtype=tf.float32,
                                              initializer=tf.contrib.layers.variance_scaling_initializer())
                self.L3_sub = tf.nn.conv2d(input=self.L2, filter=self.W3_sub, strides=[1, 1, 1, 1],
                                           padding='SAME')  # 32*32 -> 30*30
                self.L3_sub = self.batch_norm(self.L3_sub, shape=self.L3_sub.get_shape()[-1], training=self.training,
                                              convl=True, name='BN3-1')
                self.L3_sub = self.parametric_relu(self.L3_sub, 'R3_sub')
                self.W4_sub = tf.get_variable(name='W4_sub', shape=[3, 3, 160, 160], dtype=tf.float32,
                                              initializer=tf.contrib.layers.variance_scaling_initializer())
                self.L4_sub = tf.nn.conv2d(input=self.L3_sub, filter=self.W4_sub, strides=[1, 1, 1, 1],
                                           padding='VALID')  # 30*30 -> 28*28
                self.L4_sub = self.batch_norm(self.L4_sub, shape=self.L4_sub.get_shape()[-1], training=self.training,
                                              convl=True, name='BN3-2')
                self.L4_sub = self.parametric_relu(self.L4_sub, 'R4_sub')
                self.W5_sub = tf.get_variable(name='W5_sub', shape=[3, 3, 160, 160], dtype=tf.float32,
                                              initializer=tf.contrib.layers.variance_scaling_initializer())
                self.L5_sub = tf.nn.conv2d(input=self.L4_sub, filter=self.W5_sub, strides=[1, 1, 1, 1],
                                           padding='VALID')  # 28*28 -> 26*26
                self.L5_sub = self.batch_norm(self.L5_sub, shape=self.L5_sub.get_shape()[-1], training=self.training,
                                              convl=True, name='BN3-3')
                self.L5_sub = self.parametric_relu(self.L5_sub, 'R5_sub')

                # self.W3 = tf.get_variable(name='W3', shape=[3, 3, 160, 160], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
            ...
            with tf.name_scope('conv_layer4') as scope:
                self.W4 = tf.get_variable(name='W4', shape=[3, 3, 160, 320], dtype=tf.float32,
                                          initializer=tf.contrib.layers.variance_scaling_initializer())
                self.L4 = tf.nn.conv2d(input=self.L5_sub, filter=self.W4, strides=[1, 1, 1, 1],
                                       padding='SAME')  # 4*4 -> 4*4
                self.L4 = self.batch_norm(self.L4, shape=self.L4.get_shape()[-1], training=self.training, convl=True,
                                          name='BN4')
            ...
            with tf.name_scope('conv_layer5') as scope:
                # self.W5 = tf.get_variable(name='W5', shape=[3, 3, 320, 640], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                ...
                self.W6_sub = tf.get_variable(name='W6_sub', shape=[1, 3, 320, 640], dtype=tf.float32,
                                              initializer=tf.contrib.layers.variance_scaling_initializer())
                self.L6_sub = tf.nn.conv2d(input=self.L4, filter=self.W6_sub, strides=[1, 1, 1, 1],
                                           padding='SAME')  # 32*32 -> 30*30
                self.L6_sub = self.batch_norm(self.L6_sub, shape=self.L6_sub.get_shape()[-1], training=self.training,
                                              convl=True, name='BN5-1')
                self.L6_sub = self.parametric_relu(self.L6_sub, 'R6_sub')
                self.W7_sub = tf.get_variable(name='W7_sub', shape=[3, 1, 320, 640], dtype=tf.float32,
                                              initializer=tf.contrib.layers.variance_scaling_initializer())
                self.L7_sub = tf.nn.conv2d(input=self.L6_sub, filter=self.W7_sub, strides=[1, 1, 1, 1],
                                           padding='VALID')  # 30*30 -> 28*28
                self.L7_sub = self.batch_norm(self.L7_sub, shape=self.L7_sub.get_shape()[-1], training=self.training,
                                              convl=True, name='BN5-2')
            ...
            with tf.name_scope('fc_layer1') as scope:
                self.W_fc1 = tf.get_variable(name='W_fc1', shape=[4 * 4 * 640, 1000], dtype=tf.float32,
                                             initializer=tf.contrib.layers.variance_scaling_initializer())
                self.b_fc1 = tf.Variable(tf.constant(value=0.001, shape=[1000], name='b_fc1'))
                self.L_fc1 = self.batch_norm(self.L5, shape=self.L5.get_shape()[-1], training=self.training,
                                             convl=False, name='BN6')
            ...
            with tf.name_scope('fc_layer2') as scope:
                ...
                self.L_fc2 = self.batch_norm(self.L_fc1, shape=self.L_fc1.get_shape()[-1], training=self.training,
                                             convl=False, name='BN7')
            ...
            self.W_out = tf.get_variable(name='W_out', shape=[1000, 10], dtype=tf.float32,
                                         initializer=tf.contrib.layers.variance_scaling_initializer())
    ...
    def inception2d_11_33_55_maxpool11_1(self, x):
        ...
        self.W1_1 = tf.get_variable(name='W1_1', shape=[1, 1, 1, 10], dtype=tf.float32,
                                    initializer=tf.contrib.layers.variance_scaling_initializer())
        ...
        self.W1_3_1x3_sub = tf.get_variable(name='W1_3_1x3', shape=[1, 3, 1, 10], dtype=tf.float32,
                                            initializer=tf.contrib.layers.variance_scaling_initializer())
        self.L1_3_sub = tf.nn.conv2d(input=x, filter=self.W1_3_1x3_sub, strides=[1, 1, 1, 1], padding='SAME')
        self.W1_3_3x1_sub = tf.get_variable(name='W1_3_3x1', shape=[3, 1, 10, 10], dtype=tf.float32,
                                            initializer=tf.contrib.layers.variance_scaling_initializer())
        ...
        self.W1_4 = tf.get_variable(name='W1_4', shape=[1, 1, 1, 10], dtype=tf.float32,
                                    initializer=tf.contrib.layers.variance_scaling_initializer())
    ...
    def inception2d_11_33_55_maxpool11_2(self, x):
        ...
        # one_filter = tf.Variable(tf.truncated_normal([1, 1, in_channels, filter_count], mu, sigma))
        # one_by_one = tf.nn.conv2d(x, one_filter, strides=[1, 1, 1, 1], padding='SAME')
        self.W2_1 = tf.get_variable(name='W2_1', shape=[1, 1, 40, 40], dtype=tf.float32,
                                    initializer=tf.contrib.layers.variance_scaling_initializer())
        ...
        # three_filter = tf.Variable(tf.truncated_normal([3, 3, in_channels, filter_count], mu, sigma))
        # three_by_three = tf.nn.conv2d(x, three_filter, strides=[1, 1, 1, 1], padding='SAME')
        self.W2_2_1x3_1_sub = tf.get_variable(name='W2_2_1x3_1', shape=[1, 3, 40, 40], dtype=tf.float32,
                                              initializer=tf.contrib.layers.variance_scaling_initializer())
        self.L2_2_1_sub = tf.nn.conv2d(input=x, filter=self.W2_2_1x3_1_sub, strides=[1, 1, 1, 1], padding='SAME')
        self.W2_2_3x1_1_sub = tf.get_variable(name='W2_2_3x1_1', shape=[3, 1, 40, 40], dtype=tf.float32,
                                              initializer=tf.contrib.layers.variance_scaling_initializer())
        self.L2_2_2_sub = tf.nn.conv2d(input=self.L2_2_1_sub, filter=self.W2_2_3x1_1_sub, strides=[1, 1, 1, 1],
                                       padding='SAME')
        self.W2_2_1x3_2_sub = tf.get_variable(name='W2_2_1x3_2', shape=[1, 3, 40, 40], dtype=tf.float32,
                                              initializer=tf.contrib.layers.variance_scaling_initializer())
        self.L2_2_3_sub = tf.nn.conv2d(input=self.L2_2_2_sub, filter=self.W2_2_1x3_2_sub, strides=[1, 1, 1, 1],
                                       padding='SAME')
        self.W2_2_3x1_2_sub = tf.get_variable(name='W2_2_3x1_2', shape=[3, 1, 40, 40], dtype=tf.float32,
                                              initializer=tf.contrib.layers.variance_scaling_initializer())
        self.L2_2 = tf.nn.conv2d(input=self.L2_2_3_sub, filter=self.W2_2_3x1_2_sub, strides=[1, 1, 1, 1],
                                 padding='SAME')

        # 3x3 -> 1x3 3x1
        # five_filter = tf.Variable(tf.truncated_normal([5, 5, in_channels, filter_count], mu, sigma))
        # five_by_five = tf.nn.conv2d(x, five_filter, strides=[1, 1, 1, 1], padding='SAME')
        self.W2_3_1x3_sub = tf.get_variable(name='W2_3_1x3', shape=[1, 3, 40, 40], dtype=tf.float32,
                                            initializer=tf.contrib.layers.variance_scaling_initializer())
        self.L2_3_sub = tf.nn.conv2d(input=x, filter=self.W2_3_1x3_sub, strides=[1, 1, 1, 1], padding='SAME')
        self.W2_3_3x1_sub = tf.get_variable(name='W2_3_3x1', shape=[3, 1, 40, 40], dtype=tf.float32,
                                            initializer=tf.contrib.layers.variance_scaling_initializer())
        ...
        self.W2_4 = tf.get_variable(name='W2_4', shape=[1, 1, 40, 40], dtype=tf.float32,
                                    initializer=tf.contrib.layers.variance_scaling_initializer())
