    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            with tf.name_scope('conv_layer3') as scope:
                ...
                self.L3_sub = tf.nn.conv2d(input=self.L2, filter=self.W3_sub, strides=[1, 1, 1, 1], padding='SAME')  # 32*32 -> 30*30
                self.L3_sub = self.batch_norm(self.L3_sub, shape=self.L3_sub.get_shape()[-1], training=self.training, convl=True, name='BN3')
                self.L3_sub = self.parametric_relu(self.L3_sub, 'R3_sub')
                ...
                self.L4_sub = tf.nn.conv2d(input=self.L3_sub, filter=self.W4_sub, strides=[1, 1, 1, 1], padding='VALID')    # 30*30 -> 28*28
                self.L4_sub = self.batch_norm(self.L4_sub, shape=self.L4_sub.get_shape()[-1], training = self.training, convl = True, name = 'BN4')
                ...
                self.L5_sub = tf.nn.conv2d(input=self.L4_sub, filter=self.W5_sub, strides=[1, 1, 1, 1], padding='VALID')    # 28*28 -> 26*26
                self.L5_sub = self.batch_norm(self.L5_sub, shape=self.L5_sub.get_shape()[-1], training = self.training, convl = True, name = 'BN5')
            ...
            with tf.name_scope('conv_layer4') as scope:
                ...
                self.L4 = tf.nn.conv2d(input=self.L5_sub, filter=self.W4, strides=[1, 1, 1, 1], padding='SAME') # 4*4 -> 4*4
            ...
            with tf.name_scope('conv_layer5') as scope:
                ...
                self.L5 = tf.reshape(self.L5, shape=[-1, 4 * 4 * 640])
