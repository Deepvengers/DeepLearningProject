...
class Model:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            with tf.name_scope('conv_layer1') as scope:
                # 3 3 240 300 －ㄱ
                self.W4 = tf.get_variable(name='W4', shape=[3, 3, 200, 300], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                self.L4 = tf.nn.conv2d(input=self.Inception_L2, filter=self.W4, strides=[1, 1, 1, 1], padding='SAME')
            ...
            with tf.name_scope('conv_layer2') as scope:
                ...
                self.L5 = tf.reshape(self.L5, shape=[-1, 7 * 7 * 350])

            with tf.name_scope('fc_layer1') as scope:
                self.W_fc1 = tf.get_variable(name='W_fc1', shape=[7 * 7 * 350, 1000], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
    ...
    # def learning_rate_decay(self,learning_rate):
    #     # learning rate decay
