...
class Model:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            with tf.name_scope('inception_layer2') as scope:
                ...
                Inception_L2_cp = tf.identity(self.Inception_L2)
                self.ac_cost = self.auxiliary_classifiers(Inception_L2_cp, 300, 700, 700, 'ac1')
            ...
            with tf.name_scope('fc_layer1') as scope:
                self.W_fc1 = tf.get_variable(name='W_fc1', shape=[4 * 4 * 400, 650], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
            ...
            with tf.name_scope('fc_layer3') as scope:
                self.W_fc3 = tf.get_variable(name='W_fc3', shape=[650, 650], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                self.b_fc3 = tf.Variable(tf.constant(value=0.001, shape=[650], name='b_fc3'))
    ...
    def auxiliary_classifiers(self, input, convl_out, fc1_out, fc2_out, name):
        with tf.variable_scope(name):
            ...
            ac_cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=ac_logits, labels=self.Y)) + 0.0001 * reg_loss
