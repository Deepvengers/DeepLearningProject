...
from p03_rnn_stock_prediction.BNlstm import BNLSTMCell
    ...
    def _build_net(self):
        with tf.variable_scope(self.model_name):
            ...
            with tf.name_scope('conv_layer'):
                ...
                self.W3_conv = tf.get_variable(name='W3_conv', shape=[1, 10, 100, 200], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                ...
                self.W5_conv = tf.get_variable(name='W5_conv', shape=[1, 10, 200, 300], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                ...
                # self.L5_conv = tf.reshape(self.L5_conv, [-1, 20 * 20 * 300])
                ...
                self.avg_pool = tf.nn.avg_pool(value=self.L5_conv, ksize=[1, 20, 20, 1], strides=[1, 20, 20, 1], padding='VALID')
                self.avg_pool = tf.reshape(self.avg_pool, shape=[-1, 1 * 1 * 300])

            with tf.name_scope('fc_layer'):
                self.W1_fc = tf.get_variable(name='W1_fc', shape=[1 * 1 * 300, 600], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                self.b1_fc = tf.Variable(tf.constant(value=0.001, shape=[600], name='b1_fc'))
                self.L1_fc = tf.matmul(self.avg_pool, self.W1_fc) + self.b1_fc
                self.L1_fc = self.BN(input=self.L1_fc, training=self.training, name='L1_fc_BN')
                self.L1_fc = self.parametric_relu(self.L1_fc, 'R1_fc')

                self.W2_fc = tf.get_variable(name='W2_fc', shape=[600, 400], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
                self.b2_fc = tf.Variable(tf.constant(value=0.001, shape=[400], name='b2_fc'))
            ...
            self.W_out = tf.get_variable(name='W_out', shape=[400, 400], dtype=tf.float32, initializer=tf.contrib.layers.variance_scaling_initializer())
